...
def main():
    ...
    while True:  # Run a player's turn.
        ...
        playerMove = get_player_move(playerTurn, gameBoard)
        gameBoard[playerMove] = playerTurn
        # Check for a win or tie:
        if is_winner(playerTurn, gameBoard):
            display_board(gameBoard)  # Display the board one last time.
            print("Player {} has won!".format(playerTurn))
            sys.exit()
        elif is_full(gameBoard):
            display_board(gameBoard)  # Display the board one last time.
...
def get_player_move(playerTile, board):
